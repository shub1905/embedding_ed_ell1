...
def final_4d_metric(blocks_shifts_x, R_vals, possible_s, random_s_block):
    ...
    for block_id in xrange(len(blocks_shifts_x)):
        for s_id in xrange(len(blocks_shifts_x[block_id])):
            ...
            r = len(I_arr)
            for u in xrange(r):
                psi_u_all_v = numpy.bincount([randomness.custom_hash(s, u, ''.join([blocks_shifts_x[block_id][s_id][j][ri] for ri in I_arr[u]])) for j in xrange(s)])[1:] * 1.0 / (2 * r)
...
def driver_function(Data, options):
    ...
    file_name = 'distances/distances_time_{}_{}_{}_{}.data'.format(data_size, data_dim, delta, options.file_suffix)
...
def option_parsing(print_usage=False):
    ...
    parser.add_option("-a", "--alphabet", dest="alphabet_size", default=2, type='int', help='include files matching all of these regex')
    parser.add_option("-d", "--delta", dest="delta", default='0.1', type='float', help='delta, default = 0.1')
    parser.add_option("-n", "--size", dest="size", default='10', type='int', help='size of data, default = 10')
    parser.add_option("-m", "--dim", dest="dim", default='10', type='int', help='dimention, default = 10')
    parser.add_option("-s", "--suffix", dest="file_suffix", default='', help='distortion file suffix')
    parser.add_option("-t", "--typo", dest="typos", default='5', type='int', help='number of typos in string only valid if data=typo, default = 5')
...
if __name__ == '__main__':
    ...
    if options.data == 'random':
        Data = data_generation.random_data_generation(options.size, options.dim, options.alphabet_size)
